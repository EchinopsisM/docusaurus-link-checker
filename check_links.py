...
def _detect_site_domain():
    """Try to read the site URL from docusaurus.config.mjs or docusaurus.config.js."""
    for name in ('docusaurus.config.mjs', 'docusaurus.config.js', 'docusaurus.config.ts'):
...
def main():
    global SITE_DOMAIN, SITE_BASE_URL

    parser = argparse.ArgumentParser(
        description='Docusaurus link checker',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Modes:\n"
            "  local  — build the site locally and check source docs + build output\n"
            "  live   — crawl the live site and check all links\n"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument('--site-domain', default='',
                        help='Your site domain, e.g. docs.mysite.com (auto-detected from '
                             'docusaurus.config.* if omitted)')
    ...
    # ── Resolve site domain ──
    SITE_DOMAIN = args.site_domain or _detect_site_domain()
    if not SITE_DOMAIN:
        print("WARNING: Could not determine site domain. Self-referential links will not "
              "be checked against the local build.\n"
              "Pass --site-domain your-site.com to enable this check.")
    SITE_BASE_URL = f"https://{SITE_DOMAIN}" if SITE_DOMAIN else ''

    # ── Mode selection ──
    mode = args.mode
    if mode is None:
        print("=== Docusaurus Link Checker ===\n")
        print("Which site do you want to check?")
        print("  1) local  — build locally and check source docs + build output")
        live_label = f"the live site at {SITE_DOMAIN}" if SITE_DOMAIN else "the live site"
        print(f"  2) live   — fetch {live_label}\n")
        try:
