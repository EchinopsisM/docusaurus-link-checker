...
Usage:
    python check_live_links.py --site-domain your-site.com [--threads N] [--max-pages N]

Output:
    link-reports/live_links_audit.md   — human-readable report
"""
...
SITE_BASE    = ""   # set via --site-domain CLI arg
SITEMAP_URL  = ""   # derived from SITE_BASE
...
def main():
    global SITE_BASE, SITEMAP_URL

    parser = argparse.ArgumentParser(description='Check links on the live docs site.')
    parser.add_argument('--site-domain', required=False, default='',
                        help='Live site domain, e.g. docs.mysite.com (required for live mode)')
    ...
    if not args.site_domain:
        parser.error('--site-domain is required, e.g. --site-domain docs.mysite.com')
    SITE_BASE   = f"https://{args.site_domain}"
